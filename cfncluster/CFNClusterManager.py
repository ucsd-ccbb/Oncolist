...
config_file="/home/ec2-user/.cfncluster/config"

def install_cfn_cluster():
    print os.popen("sudo pip install cfncluster").read()
...
def make_config_file():
    ...
    filewriter.write("s3_read_resource = arn:aws:s3:::ucsd-ccbb-wgs-test-us-east-1" + "\n")
    filewriter.write("s3_read_write_resource = arn:aws:s3:::ucsd-ccbb-wgs-test-us-east-1/*" + "\n")
    filewriter.write("post_install = s3://ucsd-ccbb-wgs-test-us-east-1/WGS_Pipeline_Code/postinstall.sh" + "\n")

    filewriter.write("[vpc ucsd]" + "\n")
    filewriter.write("master_subnet_id = subnet-5abf8e1c" + "\n")
    filewriter.write("vpc_id = vpc-0dbe4168" + "\n")
...
def view_cfncluster_config():
    ...
    os.popen("cp config " + config_file)
...
def config_s3_resource(s3_read_resource, s3_read_write_resource):
    if not os.path.isfile(config_file):
        make_config_file()
...
def config_post_install(post_install="s3://ucsd-ccb-data-analysis/Guorong/TCGACode/postinstall.sh"):
    if not os.path.isfile(config_file):
        make_config_file()
...
def config_vpc_subnet_id(master_subnet_id="subnet-5abf8e1c",
                       vpc_id="vpc-0dbe4168"):
    if not os.path.isfile(config_file):
        make_config_file()
