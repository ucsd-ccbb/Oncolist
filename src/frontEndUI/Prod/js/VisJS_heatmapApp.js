...
app.controller('myCtrl', function ($scope, $http, $log, $filter, HttpServiceJsonp, $location, $timeout, $window) {
    // variables used for Vis JS clustering
    var clusterIndex = 0;
    var clusters = [];
...
//            improvedLayout:false,
      ...
      var data = new vis.DataSet(pre_rendered_options);
      ...
      var myrequest = HttpServiceJsonp.jsonp(url)
           .success(function (result) {
             ...
             for(var i=0; i<result.nodes.length; i++){
               ...
               var font_size = result.nodes[i].degree * 1;
               font_size = font_size < 10 ? 10 : font_size;
               if($scope.geneList.indexOf(result.nodes[i].id) > -1) {
                   geneColorArray.push(rgbString + "query");
                 nodeArray.push({id: i, label: result.nodes[i].id, title: 'Testing', x: result.nodes[i].x, y: result.nodes[i].y, font: {size: 20}, permaLabel: result.nodes[i].id, borderWidth: 4, color: {'background':rgbString, 'border': '#0000FF'}, permaColor: {'background':rgbString, 'border': '#0000FF'}, shape: 'dot', size: 16, permaSize: 20});
                 $scope.dropdownGeneNodes.push({id: i, label: result.nodes[i].id, color: rgbString});
               } else {
                   geneColorArray.push(rgbString);
                 nodeArray.push({id: i, label: result.nodes[i].id, title: 'Testing', x: result.nodes[i].x, y: result.nodes[i].y, font: {size: font_size}, color: {'background':rgbString, 'border': '#c0c0c0'}, permaColor: {'background':rgbString, 'border': '#c0c0c0'}, shape: 'dot', size: node_degree, permaSize: node_degree});
             ...
             var geneClust = {};
             var queryGeneClust = {};
             geneColorArray.map( function (a) {
                 var matchThis = a.replace("query", "");
                 if (matchThis in geneClust)
                    geneClust[matchThis]++;
                 else
                    geneClust[matchThis] = 1;

                if (a.indexOf("query") > -1){
                    if (matchThis in queryGeneClust)
                       queryGeneClust[matchThis]++;
                    else
                       queryGeneClust[matchThis] = 1;
                }
            });
             ...
             for (var key in geneClust) {
               ...
               var community_title = "Community " + alphaArray[k] + " (" + geneClust[key] + ")";

               if(key === "rgb(0,120,207)"){
                   legendNodeArray.push({id: "leg_" + k.toString(), label: "", title: "", color: key, font: {size: 30}, shape: 'square', permaColor: key, permaSize: 20, x: 300, y: 50 * k, fixed: true});
                   legendNodeArray.push({id: "leg_text_" + k.toString(), label: community_title, title: "",
                    color: key, font: {size: 30}, shadow: false, shape: 'text', permaColor: key, permaSize: 20, x: 410, y: 50 * k, fixed: true});
               } else {
                   legendNodeArray.push({id: "leg_" + k.toString(), label: "", title: "", color: key, font: {size: 30}, permaColor: key, permaSize: 20, x: 300, y: 50 * k, fixed: true});
                   legendNodeArray.push({id: "leg_text_" + k.toString(), label: community_title, title: "",
                    color: key, font: {size: 30}, shadow: false, shape: 'text', permaColor: key, permaSize: 20, x: 410, y: 50 * k, fixed: true});

                    if(key in queryGeneClust){
                        k++;
                        legendNodeArray.push({id: "legx_" + k.toString(), label: "", title: "", color: key, font: {size: 30}, shape: 'square', permaColor: key, permaSize: 20, x: 330, y: 50 * k, fixed: true});
                        legendNodeArray.push({id: "legx_text_" + k.toString(), label: "Query gene          ", title: "",
                         color: key, font: {size: 30}, shadow: false, shape: 'text', permaColor: key, permaSize: 20, x: 440, y: 50 * k, fixed: true});
                    }
               }
               k++;
             }
             ...
             var legendContainer = document.getElementById('legendVis');
             $('#legendVis').css('height', 50 * k );
             var legenedData = {
            ...
            $scope.legendNetwork = new vis.Network(legendContainer, legenedData, options);
            //$scope.legendNetwork.fit();
            var legendNodeId = "";

            $scope.legendNetwork.on("blurNode", function (params) {
                for (var nodeId in $scope.allNodes) {
                    $scope.allNodes[nodeId].color.border = $scope.allNodes[nodeId].color.background;
            ...
            $scope.legendNetwork.on("hoverNode", function (params) {
                if(params.node != null){
                    if(params.node.indexOf("text") > 0){
                        legendNodeId = params.node.replace("text_", "");
                    } else {
                        legendNodeId = params.node;
                    }
                }

                for (var nodeId in $scope.allNodes) {
                    if($scope.allNodes[nodeId].color.background === $scope.allLegendNodes[legendNodeId]["color"]){
                        $scope.allNodes[nodeId].color.border = "#000000";
                        $scope.allNodes[nodeId].color.borderWidth = 12;
                        if($scope.allNodes[nodeId].size < $scope.allNodes[nodeId].permaSize + 15){
                            $scope.allNodes[nodeId].size = $scope.allNodes[nodeId].size + 10;
                        }
                    } else {
                        $scope.allNodes[nodeId].color.border = $scope.allNodes[nodeId].color.background;
                        $scope.allNodes[nodeId].color.borderWidth = 1;
             ...
             //console.log($scope.ClusterByColor);
             //=============================
             ...
             for(var i=0; i<result.links.length; i++){
              ...
              var options = {};
              $scope.network = new vis.Network(container, data, pre_rendered_options); //vizOptions);
              ...
              $scope.network.fit();
                ...
                //$scope.network.clusterByConnection(1);
...
*/
    ...
    $scope.setNewLayout = function(layoutType){
        var options = {
          nodes: {
              borderWidth: 1,
              shape: 'dot',
        ...
        var defaultOptions = {
          ...
          for (var property in $scope.allNodes) {
              if ($scope.allNodes.hasOwnProperty(property)) {
                $scope.allNodes[property].x = undefined;
                $scope.allNodes[property].y = undefined;

                $scope.nodeArray[property].x  = undefined;
                $scope.nodeArray[property].y  = undefined;
              }
          }

          $scope.info.doneRendering = false;
        ...
        if(layoutType === "BARNSHUT"){
            $scope.network.setOptions(options);
        } else if(layoutType === "REPULSION"){
            $scope.network.setOptions(defaultOptions);
        } else if(layoutType === "HEIRARCHICALREPULSION"){
            $scope.network.setOptions(defaultOptions);
