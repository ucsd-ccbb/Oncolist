		...
		function _run(index,init){
			...
			fn.closed = function(){
				$('#divbox_frame').animate({
					...
					if(typeof(_cfg._closed)=='function') _cfg._closed(this);
				});
			}
			...
			fn.init = function(){
				if(typeof(_cfg._start)=='function') _cfg._start();
				var requires  = '#divbox,#divbox_frame,#divbox_content,#divbox_ajax';
				$(requires).remove();
				$('body').prepend('<div id="divbox"></div><div id="divbox_ajax"></div><div id="divbox_frame"><div class="closed" title="'+_cfg.languages.btn_close+'"></div><div id="divbox_data"><div id="divbox_content"></div><div class="prev" title="'+_cfg.languages.btn_prev+'"></div><div class="caption"></div><div class="number">10/10</div><div class="next" title="'+_cfg.languages.btn_next+'"></div></div></div>');
				$(_cfg.closed+','+_cfg.next+','+_cfg.prev+','+_cfg.number).hide();
				if(_cfg.overlay){
					$('#divbox').css({
			...
			fn.animate = function(t,l,w,h,fncallback,fnclosed, caption){
				...
				$(oFrame).removeClass('white').animate({
					...
					if(typeof(_cfg._beginLoad)=='function') _cfg._beginLoad(oContent);
					if(typeof(fncallback) == 'function') fncallback(oContent);
					if(typeof(_cfg._afterLoad) == 'function') _cfg._afterLoad(oContent);
					...
					if(_cfg.caption != false){
						if(_cfg.caption_control == false){
							$(c).css({'padding-left': '5px', 'padding-right': '5px'});	
						}
						
						if(caption != '' || _cfg.caption_control == true){
					...
					if(_cfg.caption_control == true){
						...
						$(_cfg.prev+','+_cfg.next+','+_cfg.number).css({top: btn_top}).show();
						if(index*1>0){
						...
						$(_cfg.number).html((index*1+1)+'/'+total)
						if(index*1<total - 1){
