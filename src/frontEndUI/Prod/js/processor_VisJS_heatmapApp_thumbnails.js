...
app.controller('myCtrl', function ($scope, $http, $log, $filter, HttpServiceJsonp, $location, $timeout, $window) {
  ...
  $scope.cluster = {
    ...
    $scope.openExport = function() {
      var url = $scope.pythonHost + "/ds/getheatmapgraph/" + $scope.info.clusterId + "/INVALIDGENE/200";
      $window.open(url);
    }
    ...
    $scope.init_page = function(){ //http://localhost:63343/Prototype2/prototype2C.html#?savedId=563a8343f6f4071f79300231
      ...
      var url = $scope.pythonHost + "/ds/getheatmapgraph/" + $scope.info.clusterId + "/PLACEHOLDER/200?callback=JSON_CALLBACK";
      var myrequest = HttpServiceJsonp.jsonp(url)
           .success(function (result) {
             //if((result.links.length <= 800) || (result.links.length > 5000)){
             if(result.links.length > 30000){
               ...
               for(var i=0; i<result.nodes.length; i++){
                 ...
                 if($scope.geneList.indexOf(result.nodes[i].id) > -1) {
                   nodeArray.push({id: i, label: result.nodes[i].id, x: result.nodes[i].x, y: result.nodes[i].y, title: 'Testing', permaLabel: result.nodes[i].id, color: rgbString, permaColor: rgbString, shape: 'square', size: 22});
                   ...
                   nodeArray.push({id: i, label: result.nodes[i].id, x: result.nodes[i].x, y: result.nodes[i].y, title: 'Testing', color: rgbString, permaColor: rgbString, shape: 'dot'});
               ...
               for(var i=0; i<result.links.length; i++){
                ...
                $scope.network.on("afterDrawing", function (params) {
                  ...
                  var loadTheseEdges;
                  if(edgeArray.length > 8000){
                    loadTheseEdges = edgeArray.slice(0,7999);
                  } else {
                    loadTheseEdges = edgeArray;
                  }
                  ...
                  my_form.name='myForm';
                  my_form.method='POST';
                  my_form.action='http://localhost:3000/saveImg/' + $scope.info.clusterId + '/' + $scope.info.clusterId;

                  my_tb2=document.createElement('textarea');
                  my_tb2.type='TEXT';
                  my_tb2.name='nodeArray';
                  my_tb2.value=JSON.stringify(nodeArray);
                  my_form.appendChild(my_tb2);

                  my_tb3=document.createElement('textarea');
                  my_tb3.type='TEXT';
                  my_tb3.name='edgesArray';
                  my_tb3.value=JSON.stringify(loadTheseEdges);
                  my_form.appendChild(my_tb3);

//                  var my_render_tb=document.createElement('INPUT');
//                  my_render_tb.type='TEXT';
//                  my_render_tb.name='nodeLocations';
//                  my_render_tb.value=JSON.stringify($scope.nodeArray);
//                  my_form.appendChild(my_render_tb);
 ...
 */
 ...
 //            improvedLayout:false,
       ...
       var url = $scope.pythonHost + "/ds/getheatmapgraph/" + $scope.info.clusterId + "/INVALIDGENE/200?callback=JSON_CALLBACK";
       var myrequest = HttpServiceJsonp.jsonp(url)
            .success(function (result) {
              $scope.result = result;
              $scope.w = 100;
