		...
		$templateCache.put("angular-szn-autocomplete/template/default.html",
			'<ul ng-show="show" class="szn-autocomplete-results" ng-class="{loading: loading}">\n' +
				'<li szn-autocomplete-result ng-repeat="result in results" ng-class="{selected: highlightIndex == $index}">\n' +
					'<span ng-if="boldMatches" view-as-html="result.value.replace(\'~DRUG\',\'\').replace(\'~GENE\',\'\').replace(\'~DISEASE\',\'\').replace(\'~PHENOTYPE\',\'\').replace(\'~GENOME\',\'\').replace(\'~\',\'**\') | sznAutocompleteBoldMatch:query"></span>\n' +
					'<span ng-if="!boldMatches">{{result.value}}</span>\n' +
					'<span ng-if="result.value.indexOf(\'~GENE\') > -1"><button class="btn btn-success btn-xs disabled" type="button">- GENE -</button></span>\n' +
					'<span ng-if="result.value.indexOf(\'~DRUG\') > -1"><button class="btn btn-primary btn-xs disabled" type="button">- DRUG -</button></span>\n' +
					'<span ng-if="result.value.indexOf(\'~PROTEIN\') > -1"><button class="btn btn-warning btn-xs disabled" type="button">- PROTEIN -</button></span>\n' +
					'<span ng-if="result.value.indexOf(\'~DISEASE\') > -1"><button class="btn btn-primary btn-xs disabled" type="button">- CANCER -</button></span>\n' +
					'<span ng-if="result.value.indexOf(\'~GENOME\') > -1"><button class="btn btn-info btn-xs disabled" type="button">- GENOME -</button></span>\n' +
					'<span ng-if="result.value.indexOf(\'~ICD10\') > -1"><button class="btn btn-primary btn-xs disabled" type="button">- ICD10 -</button></span>\n' +
					'<span ng-if="result.value.indexOf(\'~PHENOTYPE\') > -1"><button class="btn btn-warning btn-xs disabled" type="button">- PHENOTYPE -</button></span>\n' +
				'</li>\n' +
			'</ul>'
		);
	}]);
