...
def calc_localization(Gint,genes_sfari,genes_EPI,write_file_name='localization_results',num_reps=5,num_genes=20,
    ...
    kurt_SFARI, kurt_EPI =[],[]
    kurt_Srand, kurt_Erand =[],[]
    var_SFARI, var_Srand=[],[]
    var_EPI, var_Erand = [],[]
    sumTop_SFARI, sumTop_EPI = [],[]
    sumTop_Srand, sumTop_Erand = [],[]
    for r in range(num_reps):
        print(r)
        
        subset_SFARI = np.random.choice(seed_SFARI,size=num_genes_S,replace=replace)

        Fnew_SFARI = network_propagation(Gint,Wprime,subset_SFARI,alpha=.5,num_its=20)
        Fnew_SFARI.sort()
        kurt_SFARI.append(scipy.stats.kurtosis(Fnew_SFARI))
        var_SFARI.append(np.var(Fnew_SFARI))
        sumTop_SFARI.append(np.sum(Fnew_SFARI.head(1000)))

        subset_EPI = np.random.choice(seed_EPI,size=num_genes_E,replace=replace)

        Fnew_EPI = network_propagation(Gint,Wprime,subset_EPI,alpha=.5,num_its=20)
        Fnew_EPI.sort()
        kurt_EPI.append(scipy.stats.kurtosis(Fnew_EPI))
        var_EPI.append(np.var(Fnew_EPI))
        sumTop_EPI.append(np.sum(Fnew_EPI.head(1000)))
        ...
        Fnew_Srand = network_propagation(G_rand,Wprime_rand,subset_SFARI,alpha=.5,num_its=20)
        Fnew_Srand.sort()
        ...
        Fnew_Erand = network_propagation(G_rand,Wprime_rand,subset_EPI,alpha=.5,num_its=20)
        Fnew_Erand.sort()
        kurt_Erand.append(scipy.stats.kurtosis(Fnew_Erand))
        var_Erand.append(np.var(Fnew_Erand))
        sumTop_Erand.append(np.sum(Fnew_Erand.head(1000)))
    ...
    results_dict = {'kurt_SFARI':kurt_SFARI,'kurt_Srand':kurt_Srand,
                   'kurt_EPI': kurt_EPI, 'kurt_Erand':kurt_Erand,
                   'var_SFARI':var_SFARI,'var_Srand':var_Srand,
                   'var_EPI':var_EPI,'var_Erand':var_Erand,
                   'sumTop_SFARI':sumTop_SFARI, 'sumTop_Srand':sumTop_Srand,
                   'sumTop_EPI':sumTop_EPI, 'sumTop_Erand':sumTop_Erand,
...
def NBS_cluster_heat_vectors(G_int,mutation_dict,num_reps=2,num_samps = 80,
                             save_file=True,write_file='temp_NBS.csv'):
    
    patient_ids = mutation_dict.keys()

    num_nodes = len(G_int.nodes())
    num_patients = len(patient_ids)
    
    # map from patient id to index (numeric) and back
    idx_to_id = dict(zip(range(num_patients),patient_ids))
    id_to_idx = dict(zip(patient_ids,range(num_patients)))
    
    Wprime = normalized_adj_matrix(G_int)
    
    # initialize same group for output
    same_group = np.zeros([num_patients,num_patients])
    
    for r in range(num_reps):
        print(r)
        print 'propagating heat from each patient"s mutations...'
        Fnew_dict = heat_from_mutations(G_int,Wprime,mutation_dict,num_samps=num_samps)
    
    
        patient_gene_Cmat = np.zeros([num_samps,num_nodes])
        count = -1
        for key in Fnew_dict.keys():
            count = count+1
            patient_gene_Cmat[count,:] = Fnew_dict[key]


        # symmetrize patient_gene_Cmat
        print 'symmetrizing patient-gene matrix...'
        patient_gene_sym = np.zeros([num_samps+num_nodes,num_samps+num_nodes])
        patient_gene_sym[0:num_samps,num_samps:] = patient_gene_Cmat
        patient_gene_sym[num_samps:,0:num_samps] = np.transpose(patient_gene_Cmat)

        node_list = list(Fnew_dict.keys())
        node_list.extend(G_int.nodes())
        idx_to_id_sym = dict(zip(range(num_samps+num_nodes),node_list))

        G_nbs = nx.from_numpy_matrix(patient_gene_sym)
        G_nbs = nx.relabel_nodes(G_nbs,idx_to_id_sym)

        print 'clustering patient-gene matrix...'
        partition_sym = community.best_partition(G_nbs)
        partition_sym = pd.Series(partition_sym)

        # find out how many times node i is in the same group as node j
        print 'calculating cluster co-occurence for each patient pair...'
        for key1 in Fnew_dict.keys():
            group_1 = partition_sym[key1]
            for key2 in Fnew_dict.keys():
                group_2 = partition_sym[key2]

                if group_1==group_2:
                    same_group[id_to_idx[key1],id_to_idx[key2]] = same_group[id_to_idx[key1],id_to_idx[key2]]+1

    G_nbs_2 = nx.from_numpy_matrix(same_group)
    G_nbs_2 = nx.relabel_nodes(G_nbs_2,idx_to_id)
    partition_2 = community.best_partition(G_nbs_2)
    
    partition_2 = dict(partition_2)

    idx_sort_p = np.argsort(partition_2.values())

    keys = partition_2.keys()
    keys_sort = [keys[i] for i in idx_sort_p]
    values = partition_2.values()

    keys_mapped = [id_to_idx[key] for key in keys_sort]
    NBS_sort = same_group[:,keys_mapped]
    NBS_sort = NBS_sort[keys_mapped,:]
    
    if save_file:
        # save file as a dataframe
        NBS_sort_df = pd.DataFrame(NBS_sort,columns=keys_sort,index=keys_sort)
        NBS_sort_df.to_csv(write_file,sep='\t')

    return NBS_sort, keys_sort

    
    
def heat_from_mutations(Gint,Wprime,mutation_dict,num_samps = 80):
    Fnew_dict = dict()

    patient_ids = mutation_dict.keys()
    focal_id_subsample = list(np.random.choice(patient_ids,size=num_samps,
                                              replace=False))
    
    for focal_id in focal_id_subsample:
        print(focal_id)
        focal_seed = mutation_dict[focal_id].keys() #AEM_clinical['De-novo Mutations'][focal_id]
        focal_seed = np.intersect1d(Gint.nodes(),focal_seed)

        Fnew_focal = network_propagation(Gint,Wprime,focal_seed)
        Fnew_dict[focal_id] = Fnew_focal
