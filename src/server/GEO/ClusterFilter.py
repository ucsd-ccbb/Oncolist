...
def deduplicate_cluster(cluster_file_dir, project_name):
    ...
    for gamma in [1, 4, 7, 11, 14, 17, 20]:
        cluster_folder = cluster_file_dir + "/" + project_name + "_gamma_" + str(gamma)
        cluster_file = cluster_folder + "/" + "rnaseq_vs_rnaseq_gamma_" + str(gamma) + ".tsv"
    ...
    print cluster_file_dir + " " + project_name + ": total similar cluster: " + str(len(similar_cluster_id))
    print cluster_file_dir + " " + project_name + ": total unique cluster: " + str(len(uniq_cluster_id))
    return uniq_cluster_id

def print_unique_cluster(cluster_file_dir, unique_cluster_file_dir, project_name, uniq_cluster_id):
    for gamma in [1, 4, 7, 11, 14, 17, 20]:
        cluster_folder = cluster_file_dir + "/" + project_name + "_gamma_" + str(gamma)
        cluster_file = cluster_folder + "/" + "rnaseq_vs_rnaseq_gamma_" + str(gamma) + ".tsv"
        unique_cluster_folder = unique_cluster_file_dir + "/" + project_name + "_gamma_" + str(gamma)
        unique_cluster_file = unique_cluster_folder + "/" + "rnaseq_vs_rnaseq_gamma_" + str(gamma) + ".unique.tsv"
...
if __name__ == "__main__":
    ...
    #cluster_file_dir = "/Users/guorongxu/Desktop/SearchEngine/GEO/cluster_files"
    #unique_cluster_file_dir = "/Users/guorongxu/Desktop/SearchEngine/GEO/unique_cluster_files"
    #project_name = "GSE44295_Yogen_Uveal_Melanoma_63_Arrays_Cleveland"

    uniq_cluster_id = deduplicate_cluster(cluster_file_dir, project_name)
    print_unique_cluster(cluster_file_dir, unique_cluster_file_dir, project_name, uniq_cluster_id)
